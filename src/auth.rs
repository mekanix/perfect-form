...
pub async fn current_user(cx: &Cx, db: &toasty::Db, cfg: &Config) -> Result<Option<user::Model>> {
    ...
    let mut db = db.clone();
    Ok(Some(user::Model::get_by_id(&mut db, &user_id).await?))
}
...
pub async fn require_roles(
    ...
    let mut db = db.clone();
    let roles: Vec<crate::entity::role::Model> = user.roles().exec(&mut db).await?;
