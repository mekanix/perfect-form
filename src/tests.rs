...
async fn setup() -> (Router, Config, toasty::Db) {
    let cfg = Config {
...
async fn create_user(db: &toasty::Db, email: &str, admin: bool) -> user::Model {
    user::Model::create()
        .email(email)
...
async fn create_role(db: &toasty::Db, name: &str) -> role::Model {
    role::Model::create()
        .name(name)
        .exec(&mut db.clone())
        .await
...
async fn assign_role(db: &toasty::Db, user_id: i32, role_id: i32) {
    users_roles::Model::create()
        .user_id(user_id)
        .role_id(role_id)
        .exec(&mut db.clone())
        .await
...
#[tokio::test]
async fn login_with_wrong_password_renders_toast_oob() {
    let (router, _cfg, db) = setup().await;
    let _user = create_user(&db, "login-test@example.com", false).await;

    let body = "email=login-test%40example.com&password=wrong-password";
    let response = router.handle(post_form("/login", body)).await;

    assert_eq!(response.status(), 200);

    let bytes = topcoat::router::to_bytes(response.into_body(), 1024 * 1024)
        .await
        .unwrap();
    let html = String::from_utf8(bytes.to_vec()).unwrap();
    assert!(html.contains(
        r#"<div id="toast" class="visible" hx-swap-oob="true">Invalid email or password</div>"#
    ));
}
